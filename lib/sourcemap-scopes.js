  ...
  for (const node of toplevel.body) {
    node.walk(new TreeWalker((node, descend) => {
      if (node.is_block_scope()) {
        const variableNodes = [...node.block_scope.variables?.values()];
        scopeInfoBuilder.startScope(node.block_scope.start.line, node.block_scope.start.col, {
          kind: "block",
          variables: variableNodes.map(v => v.name),
        });
        const scope = scopeInfoBuilder.currentScope();
        scope.variableIds = variableNodes.map(v => v.original_id);
        node.original = { scope };
      ...
      if (node instanceof AST_Scope && !(node instanceof AST_DefClass) && !(node instanceof AST_ClassExpression)) {
        ...
        const scope = scopeInfoBuilder.currentScope();
        if (node.variables) {
          const variableNodes = filterUnusedArgumentsVar(node.variables);
...
export function process_generated_ranges(toplevel, scopeInfoBuilder) {
  ...
  const startNode = toplevel.body[0];
  scopeInfoBuilder.startRange(startNode.gen_start.line, startNode.gen_start.col, { scope: scopeInfoBuilder.lastScope() });
  ...
  scopeInfoBuilder.endRange(endNode.gen_end.line, endNode.gen_end.col);
}
