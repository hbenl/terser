  ...
  for (let i = startIndex; i < toplevel.body.length; ++i) {
    ...
    const tw = new TreeWalker((node, descend) => {
      if (node.is_block_scope()) {
        ...
        scope.kind = "block";
        const variables = [...node.block_scope.variables?.values()];
        scope.variables = variables.map(v => v.name);
        scope.variableIds = variables.map(v => v.original_id);
      ...
      if (node instanceof AST_Scope && !(node instanceof AST_DefClass) && !(node instanceof AST_ClassExpression)) {
        ...
        scope.kind =
            ...
            : undefined;
        if (node.variables) {
          const variables = filterUnusedArgumentsVar(node.variables);
          scope.variables = variables.map(v => v.name);
          scope.variableIds = variables.map(v => v.original_id);
        }
...
export function process_generated_ranges(toplevel) {
  const result = [];
  for (const [, , scope] of toplevel.originalScopes) {
    const startIndex = 0;
    const endIndex = toplevel.body.length;
    const startNode = toplevel.body[startIndex];
    ...
    for (let i = startIndex; i < endIndex; ++i) {
      ...
      const tw = new TreeWalker((node, descend) => {
        const values = node.original?.scope.variableIds?.map(id => SymbolDef.generated_names.get(id));
        if (node.is_block_scope()) {
          const range = { ...baseRangeFromNode(node), values };
...
function baseRangeFromNode(node) {
  ...
  if (node.original?.callsite) {
    ...
    callsite = {
      sourceIdx: 0, // TODO use callsite.file to compute sourceIdx
      line,
      column,
    };
  }
  return {
    ...
    originalScope: node.original?.scope,
    callsite,
    children: [],
  };
}