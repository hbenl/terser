...
export function process_original_scopes(options_parse, scopeInfoBuilder) {
  ...
  if (!scopeInfoBuilder.toplevelScopes) {
    scopeInfoBuilder.toplevelScopes = [];
  }
  scopeInfoBuilder.toplevelScopes.push(scope);
...
export function process_generated_ranges(toplevel, scopeInfoBuilder) {
  for (const scope of scopeInfoBuilder.toplevelScopes) {
    const values = [];
    if (scope?.variables) {
  ...
  for (let i = 0; i < scopeInfoBuilder.toplevelScopes.length; i++) {
